...
def new_request(passed_recaptcha = False, data = None):
	if request.method == 'POST' or data:
		...
		if is_spam(request_text) and not passed_recaptcha:
			return render_template('recaptcha.html', form = data, message = "Hmm, our automated system thinks this request looks a bit too much like spam.  Just type the numbers or letters you see in the field below and we will submit your request in full.", public_key = app.config['RECAPTCHA_PUBLIC_KEY'])
		alias = None
		phone = None
