...
def add_resource(resource, request_body, current_user_id=None):
    ...
    if "note" in resource:
        ...
        return add_note(request_id=int(fields['request_id']), text=fields['note_text'], user_id=current_user_id, passed_spam_filter=True)
        ...
        participant_id, new = add_staff_participant(request_id=fields[
                                                    'request_id'], email=fields['owner_email'], reason=fields['owner_reason'])
        if new:
            generate_prr_emails(request_id=fields['request_id'], notification_type="Staff participant added", user_id=get_attribute(
                "user_id", obj_id=participant_id, obj_type="Owner"))
...
def update_resource(resource, request_body):
    ...
    if "owner" in resource:
        ...
        update_obj(attribute="text", val=fields[
                   'request_text'], obj_type="Request", obj_id=fields['request_id'])
    elif "note_text" in resource:
        update_obj(attribute="text", val=fields[
                   'note_text'], obj_type="Note", obj_id=fields['response_id'])
        # Need to store note text somewhere else (or just do delete here as
        # well)
        ...
        # Need to store record somewhere else and prompt them to delete from
        # Scribd as well, if they'd like
    else:
        return False
...
def request_extension(request_id, extension_reasons, user_id):
    ...
    # Bypass spam filter because they are logged in.
    return add_note(request_id=request_id, text=text, user_id=user_id, passed_spam_filter=True)
...
def add_note(request_id, text, user_id=None, passed_spam_filter=False):
    ...
    if note_id:
        ...
        if user_id:
            add_staff_participant(request_id=request_id, user_id=user_id)
            generate_prr_emails(
                request_id=request_id, notification_type="City response added")
        else:
            generate_prr_emails(
                request_id=request_id, notification_type="Public note added")
        return note_id
    return False
...
def upload_record(request_id, description, user_id, document=None):
    ...
    try:
        ...
        doc_id, filename = scribd_helpers.upload_file(
    ...
    if doc_id == False:
        ...
        if doc_id:
            ...
            record_id = create_record(doc_id=None, request_id=request_id, user_id=user_id,
                                      description=description, filename=filename, url=app.config['HOST_URL'] + doc_id)
            change_request_status(request_id, "A response has been added.")
            generate_prr_emails(
                request_id=request_id, notification_type="City response added")
            add_staff_participant(request_id=request_id, user_id=user_id)
...
def add_link(request_id, url, description, user_id):
    ...
    record_id = create_record(
        url=url, request_id=request_id, user_id=user_id, description=description)
    if record_id:
        change_request_status(request_id, "A response has been added.")
        generate_prr_emails(
            request_id=request_id, notification_type="City response added")
...
def make_request(text, email=None, user_id=None, phone=None, address1=None, address2=None, city=None, state=None, zipcode=None, alias=None, department=None, passed_spam_filter=False, offline_submission_type=None, date_received=None):
    ...
    if department:
        ...
        if prr_email:
            ...
            department = None
    # Actually create the Request object
    request_id = create_request(
        text=text, user_id=user_id, offline_submission_type=offline_submission_type, date_received=date_received)
    ...
    # Set the status of the incoming request to "Open"
    open_request(request_id)
    if email or alias or phone:
        subscriber_user_id = create_or_return_user(
            email=email, alias=alias, phone=phone, address1=address1, address2=address2, city=city, state=state, zipcode=zipcode)
...
def add_subscriber(request_id, email):
    ...
    if subscriber_id:
        generate_prr_emails(
            request_id, notification_type="Request followed", user_id=user_id)
...
def ask_a_question(request_id, user_id, question):
    ...
    qa_id = create_QA(
        request_id=request_id, question=question, user_id=user_id)
    if qa_id:
        change_request_status(request_id, "Pending")
        requester = req.requester()
        if requester:
            generate_prr_emails(
                request_id, notification_type="Question asked", user_id=requester.user_id)
        add_staff_participant(request_id=request_id, user_id=user_id)
        return qa_id
    return False
...
def answer_a_question(qa_id, answer, subscriber_id=None, passed_spam_filter=False):
    ...
    if (not answer) or (answer == "") or (not passed_spam_filter):
        ...
        # We aren't changing the request status if someone's answered a
        # question anymore, but we could change_request_status(request_id,
        # "Pending")
        generate_prr_emails(
            request_id=request_id, notification_type="Question answered")
        return True
...
def assign_owner(request_id, reason, email=None):
    ...
    owner_id, is_new_owner = add_staff_participant(
        request_id=request_id, reason=reason, email=email, is_point_person=True)
    ...
    update_obj(
        attribute="department_id", val=new_owner.user.department, obj=req)
    user_id = get_attribute(
        attribute="user_id", obj_id=owner_id, obj_type="Owner")
    # Send notifications
    if is_new_owner:
        generate_prr_emails(
            request_id=request_id, notification_type="Request assigned", user_id=user_id)
    return owner_id
...
def get_request_data_chronologically(req):
    ...
    for i, note in enumerate(req.notes):
        if not note.user_id:
            responses.append(
                RequestPresenter(note=note, index=i, public=public, request=req))
    for i, qa in enumerate(req.qas):
        responses.append(
            RequestPresenter(qa=qa, index=i, public=public, request=req))
    if not responses:
        return responses
...
def get_responses_chronologically(req):
    ...
    if "Closed" in req.status:
        # Set most recent note (closed note)'s icon
        responses[0].set_icon("icon-archive")
    return responses
...
def set_directory_fields():
    ...
    if 'STAFF_URL' in app.config:
        # This gets run at regular internals via db_users.py in order to keep
        # the staff user list up to date. Before users are added/updated, ALL
        # users get reset to 'inactive', and then only the ones in the current
        # CSV are set to active.
        ...
        for row in dictreader:
            create_or_return_user(email=row['email'].lower(), alias=row['name'], phone=row[
                                  'phone number'], department=row['department name'], is_staff=True)
        ...
        if 'LIAISONS_URL' in app.config:
            ...
            dictreader = csv.DictReader(csvfile, delimiter=',')
            for row in dictreader:
                user = create_or_return_user(
                    email=row['PRR liaison'], contact_for=row['department name'])
                if row['PRR backup'] != "":
                    user = create_or_return_user(
                        email=row['PRR backup'], backup_for=row['department name'])
        else:
            app.logger.info(
        ...
        if 'DEFAULT_OWNER_EMAIL' in app.config and 'DEFAULT_OWNER_REASON' in app.config:
            create_or_return_user(email=app.config['DEFAULT_OWNER_EMAIL'].lower(), alias=app.config[
                                  'DEFAULT_OWNER_EMAIL'], department=app.config['DEFAULT_OWNER_REASON'], is_staff=True)
...
def close_request(request_id, reason="", user_id=None):
    ...
    generate_prr_emails(
        request_id=request_id, notification_type="Request closed")
    add_staff_participant(request_id=request_id, user_id=user_id)
