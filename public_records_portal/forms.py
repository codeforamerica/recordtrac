...
class OfflineRequestForm(Form):
    request_category = SelectField(u'Category*', choices=categories,
                                   validators=[DataRequired('The request category is required')])
    request_agency = SelectField(u'Agency*', choices=agencies, validators=[DataRequired('Please select an agency')],
                                 default='')
    request_summary = TextAreaField(u'Summary*', validators=[DataRequired('You must enter a summary of your request'),
                                                             Length(1, 250,
                                                                    'Your request summary must be less than 250 characters')])
    request_text = TextAreaField(u'Detailed Description*', validators=[
        Length(1, 5000, 'The detailed description of this request must be less than 5000 characters')])
    request_attachment_description = StringField(u'Description of Attachment')
    request_attachment = FileField(u'Upload attachment')
    request_format = SelectField(u'Format Received*', choices=formats, validators=[
        DataRequired('You must enter the format in which the request was received')], default='')
    request_date = DateField(u'Request Date*', format='%m/%d/%Y',
                             validators=[DataRequired('You must enter the date this request was received.')])
    request_first_name = StringField(u'First Name*',
                                     validators=[DataRequired('Please enter the requestor\'s first name')])
    ...
    request_address_zip = StringField(u'Zip Code',
                                      validators=[Length(5, 5, 'Please enter the your five-digit zip code')])
...
class NewRequestForm(Form):
    ...
    terms_of_use = BooleanField(
        u'I acknowledge that I have read and accepted the Terms of Use for this application, as stated above',
