"""
	...
	Implements functions that render the Jinja (http://jinja.pocoo.org/)
		templates/html for RecordTrac.
...
def new_request(passed_recaptcha=False, data=None):
	...
	if data or request.method == 'POST':
		...
		request_address = [str(data['request_address_1']) +
						   str(data['request_address_2']) +
						   str(data['request_address_city']) +
						   str(data['request_address_state']) +
						   str(data['request_address_zip'])]
		request_fax = data['request_fax']
		request_alias = data['request_alias']
		app.logger.debug('Received Data; Starting Validation')
		# Request Validation
		if not request_text or len(request_text) < app.config['MIN_REQUEST_LEN']:
			messages.append(
				"Please make sure to fill out your request description")
			print messages
		elif len(request_text) > app.config['MAX_REQUEST_LEN']:
			messages.append("Please try to simplifiy your request. It's too long")

		if current_user.is_authenticated():
			if not request_format:
				messages.append("Please select a format for the request")
			if not request_date:
				messages.append("Please enter date received for the request")
			else:
				try:
					...
					messages.append("Please use the datepicker to enter a date")
		if messages:
			print 'Hello, World!'
			if current_user.is_authenticated():
		...
		if current_user.is_authenticated():
			return render_template('offline_request.html', routing_available=routing_available, departments=departments, messages=messages)
		else:
			return render_template('offline_request.html',
			                       routing_available=routing_available, departments=departments, messages=messages)
